...
- Live-подтверждение прямого провайдера typesafe; отказ через кандидата `NONE` в адаптере (рекомендован [V0.3b](research/V0.3b-refusal.md), не реализован).
...
поднимает корректный отказ Jev до 96% без потерь на обычных задачах; перенос в адаптер —
отдельное изменение с ADR. Следующий шаг по [плану](ROADMAP.md) — V0.4.
