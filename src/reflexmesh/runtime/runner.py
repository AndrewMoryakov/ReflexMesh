...
class AttemptSupervisor:
    ...
    def __init__(self, task: ExecutionTask, strategy, verifier=None):
    ...
    def cancel(self):
        ...
        try:
            return self.gate.stop("cancel_requested")
        except RuntimeStop:
            return False

    def _verify(self, remaining: float) -> list[dict]:
        unknown = [{"id": c.id, "status": "unknown", "evidence_refs": []} for c in self.task.criteria]
    ...
    def _record(self, event, actions):
        if event[0] in ("dispatch", "driver_return", "effect_unknown", "rejected", "reobserve",
                        "decision_request", "step", "stopped", "done", "observation"):
    ...
    def run(self) -> dict:
        ...
        if status == "verifying":
            ...
            proven = {c.predicate for c, v in zip(self.task.criteria, assessments) if v == "pass"}
            ...
            validated = set().union(*(covered.get(p, set()) for p in proven))
            for row in actions:
                if row.get("operation") in validated and row.get("operation") != "navigate":
                    row["effect"] = "applied"
        ...
        outcome = ("pass" if status == "completed" else "fail" if status == "failed" and
                   reason == "postcondition_failed" else "unknown")
