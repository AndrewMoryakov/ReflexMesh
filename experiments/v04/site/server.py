"""V0.4 test site: small deterministic pages with a server-side request log for an independent verifier.

    python server.py --port 8765

GET /__state returns {"log": [...], "state": {...}}; POST /__reset clears both. Every page request
and form submission is logged with a timestamp, so the verifier checks what reached the server,
not what the harness claims. /slow/export answers after --slow-seconds.
"""
import argparse
...
NAV = ('<nav><a href="/">Home</a> | <a href="/profile">Profile</a> | <a href="/settings">Settings</a> | '
       '<a href="/reports">Reports</a> | <a href="/form">Contact form</a> | <a href="/danger">Danger zone</a> | '
       '<a href="/slow">Export</a></nav>')
...
def render(path: str) -> bytes | None:
    ...
    if path == "/settings":
        ...
        return page("Settings", f"{saved}<form method='post' action='/settings'>"
                    f"<label><input type='checkbox' name='notify_email'{checked}> Email notifications</label> "
    ...
    if path == "/danger":
        ...
        return page("Danger zone", f"{gone}<p>Deleting the account cannot be undone.</p>"
                    "<form method='post' action='/danger/delete'><button type='submit'>Delete account</button></form>")
    if path == "/slow":
        ...
        return page("Export", f"{done}<p>Exporting takes a few seconds.</p>"
                    "<form method='post' action='/slow/export'><button type='submit'>Start export</button></form>")
    return None
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _log(self, method: str, data: dict | None = None) -> None:
        with LOCK:
            LOG.append({"t": time.time(), "method": method, "path": self.path.split("?")[0], "data": data})
    ...
    def do_GET(self):
        ...
        if path == "/__state":
            with LOCK:
                return self._send(200, json.dumps({"log": LOG, "state": STATE}).encode(), "application/json")
        if path == "/favicon.ico":
